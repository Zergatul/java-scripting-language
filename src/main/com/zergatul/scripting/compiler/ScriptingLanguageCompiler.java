...
public class ScriptingLanguageCompiler {
    ...
    private Class<Runnable> compileRunnable(CompileConsumer consumer) throws ScriptCompileException {
        return compileRunnable(consumer, new VariableContextStack());
    }
    ...
    private Class<Runnable> compileRunnable(CompileConsumer consumer, VariableContextStack context) throws ScriptCompileException {
        ...
        MethodVisitorWrapper visitor = new MethodVisitorWrapper(methodVisitor, name, context);
        ...
        for (StaticVariableEntry entry : visitor.getContextStack().getStaticVariables()) {
    ...
    private void compile(ASTInput input, CompilerMethodVisitor visitor) throws ScriptCompileException {
        if (input.jjtGetNumChildren() < 1) {
            throw new ScriptCompileException("ASTInput num children=0.");
        }

        if (!(input.jjtGetChild(0) instanceof ASTStaticVariablesList list)) {
            throw new ScriptCompileException("ASTInput static vars list expected.");
        }

        compile(list, visitor);

        for (int i = 1; i < input.jjtGetNumChildren(); i++) {
    ...
    private void compile(ASTStaticVariableDeclaration declaration, CompilerMethodVisitor visitor) throws ScriptCompileException {
        ...
        VariableEntry variable = visitor.getContextStack().addStatic(visitor.getClassName(), variableName, type);
