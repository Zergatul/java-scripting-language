...
public class SearchEntry {
    ...
    public boolean isSingleWordStatementStart(int line, int column) {
        switch (node.getNodeType()) {
            case NAME_EXPRESSION: {
                if (parent.node.getNodeType() == BoundNodeType.EXPRESSION_STATEMENT) {
                    if (node.getRange().containsOrEnds(line, column)) {
                        ...
                        switch (parent.parent.node.getNodeType()) {
                            case CLASS_CONSTRUCTOR:
                                isSimpleStatement = ((BoundClassConstructorNode) parent.parent.node).syntaxNode.arrow != null;
                                break;
                            case CLASS_METHOD:
                                isSimpleStatement = ((BoundClassMethodNode) parent.parent.node).syntaxNode.arrow != null;
                                break;
                            case EXTENSION_METHOD:
                                isSimpleStatement = ((BoundExtensionMethodNode) parent.parent.node).syntaxNode.arrow != null;
                                break;
            ...
            case LET_TYPE:
            case INVALID_TYPE: {
                return parent.node.getNodeType() == BoundNodeType.VARIABLE_DECLARATION;
            }
            case PREDEFINED_TYPE: {
                ...
                return parent.node.getNodeType() == BoundNodeType.STATIC_REFERENCE;
            }
